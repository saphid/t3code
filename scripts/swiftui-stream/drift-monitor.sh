...
if [[ "$FETCH_OK" != true ]]; then
  STATUS=fetch-failed
  DETAIL="could not refresh one or more canonical SwiftUI refs"
elif [[ -z "$THEO" || -z "$DEV" || -z "$TEST" ]]; then
  STATUS=missing-ref
...
jq -n --arg status "$STATUS" --arg detail "$DETAIL" --arg theo "$THEO" --arg dev "$DEV" --arg test "$TEST" --arg checkedAt "$CHECKED_AT" \
  '{status:$status,detail:$detail,theo:$theo,dev:$dev,test:$test,checkedAt:$checkedAt}' > "$STATE_DIR/drift-monitor.next.json"
