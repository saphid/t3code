...
const make = Effect.gen(function* () {
  ...
  // Replay command id to the queued turn start it re-requests. `sent` settles once the replay's
  // provider send finishes, which lets the next queued turn follow it in order.
  ...
  const resumeTurnsAfterCompaction = Effect.fn("resumeTurnsAfterCompaction")(function* (
    ...
    while (queued.length > 0 && turnsAfterCompaction.get(threadId) === queued) {
      ...
      // Reissue the durable request after restoration clears compaction's pending slot.
      // Reusing the message id preserves a single user bubble.
    ...
    yield* Effect.gen(function* () {
      ...
      if (!latestThread) {
        return yield* appendProviderFailureActivity({
          ...
          kind: "provider.turn.start.failed",
          summary: "Provider turn start failed",
          detail: "The thread disappeared before its provider turn could start.",
          turnId: null,
          createdAt: event.payload.createdAt,
        });
      }
      ...
      if (!hasOtherUserMessages && !isCompactCommand) {
        ...
        if (canReplaceThreadTitle(latestThread.title, event.payload.titleSeed)) {
      ...
      // The forked send settles `sent` from here on, so drop the entry used by post-processing.
  ...
  const processDomainEventSafely = (event: ProviderIntentEvent) =>
    processDomainEvent(event).pipe(
      // A replay that returned before forking its send still holds its entry. Settle it so the
      // compaction queue moves on. Forked sends drop the entry first and settle it themselves.
      Effect.ensuring(
        Effect.suspend(() => {
