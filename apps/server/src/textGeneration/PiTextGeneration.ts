...
import { type PiSettings, type ModelSelection } from "@t3tools/contracts";
...
export const makePiTextGeneration = Effect.fn("makePiTextGeneration")(function* (
  ...
  const runPiJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      const resolvedModel = resolvePiBaseModelId(modelSelection.model);
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        yield* applyPiAcpModelSelection({
          ...
          mapError: (cause) =>
            new TextGenerationError({
              operation,
              detail: "Failed to set Pi ACP base model for text generation.",
              cause,
            }),
        });
        ...
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new TextGenerationError({ operation, detail: "Pi ACP request timed out." }),
              ),
