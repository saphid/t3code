...
export const advertisedHostFlag = Flag.string("advertised-host").pipe(
  Flag.withDescription(
    "HTTP(S) address to advertise to phones when it differs from the bind host.",
  ),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  ...
  advertisedHost: Config.string("T3CODE_ADVERTISED_HOST").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
...
export const sharedServerCommandFlags = {
  ...
  port: portFlag,
  host: hostFlag,
  advertisedHost: advertisedHostFlag,
  baseDir: baseDirFlag,
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const normalizedFlags = {
      ...
      advertisedHost: flags.advertisedHost ?? Option.none(),
    ...
    const advertisedHost = Option.getOrUndefined(
      resolveOptionPrecedence(
        normalizedFlags.advertisedHost,
        Option.fromUndefinedOr(env.advertisedHost),
      ).pipe(Option.filter((value) => value.trim().length > 0)),
    );
