...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const prepareReviewIndex = Effect.fn("prepareReviewIndex")(function* (
    ...
    if (indexExists) {
      ...
      // A fresh copy timestamp makes Git trust stale stat data for same-size edits.
      // Round below the source timestamp so Git retains its racy-index check.
      const indexTime = Option.isSome(mtime)
        ? Math.max(1, Math.floor((mtime.value.getTime() - 1) / 1000))
        : 1;
