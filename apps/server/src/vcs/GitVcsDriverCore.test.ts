...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("remote operations", () => {
    it.effect("returns null when a fetched remote lacks the selected local branch", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        ...
        yield* git(cwd, ["remote", "add", "origin", remote]);
        yield* git(cwd, ["push", "-u", "origin", initialBranch]);
        yield* git(cwd, ["checkout", "-b", "local-only"]);
        ...
        assert.isFalse(
          (yield* driver.listRefs({ cwd, limit: 100 })).refs.some(
            (ref) => ref.name === "origin/local-only",
          ),
        );
      }),
    );
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        if (resolvedBase === null) {
          throw new Error("Expected the fetched remote base to resolve");
        }
