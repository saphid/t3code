...
import {
  ...
  currentPiModelIdFromSessionSetup,
  makePiAcpRuntime,
...
const resolveNotificationTurnId = (ctx: PiSessionContext): TurnId | undefined => ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: PiSessionContext): TurnId | undefined => ctx.activeTurnId;
...
export function piPromptSettlementBelongsToContext(input: {
  ...
  return Effect.gen(function* () {
    ...
    const settlePromptInFlight = (
      ...
      Effect.gen(function* () {
        ...
        const settlementBelongsToLiveContext = piPromptSettlementBelongsToContext({
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const requestedStartModelId = piModelSelection?.model
            ? resolvePiBaseModelId(piModelSelection.model)
            : undefined;
          const boundModelId = yield* applyPiAcpModelSelection({
            runtime: acp,
            currentModelId: currentPiModelIdFromSessionSetup(started.sessionSetupResult),
          ...
          const session: ProviderSession = {
            ...
            cwd,
            ...(boundModelId ? { model: resolvePiBaseModelId(boundModelId) } : {}),
            threadId: input.threadId,
            resumeCursor: {
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              ...
              const requestedTurnModelId = turnModelSelection?.model
                ? resolvePiBaseModelId(turnModelSelection.model)
                : undefined;
              ...
              const displayModel = currentModelId
                ? resolvePiBaseModelId(currentModelId)
                : undefined;
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "Pi prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  {
                    errorMessage: "Pi session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "Pi session changed before the turn completed.",
                });
              }
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "Pi prompt request failed.",
                }),
              );
