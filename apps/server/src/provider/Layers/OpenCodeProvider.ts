...
export const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (
  ...
  const loadInventoryFromServer = (serverPassword?: string) =>
    Effect.scoped(
      Effect.gen(function* () {
  ...
  const inventoryEffect = isExternalServer
    ? loadInventoryFromServer(openCodeSettings.serverPassword)
    : openCodeRuntime
        ...
        .pipe(
          Effect.catch(() => loadInventoryFromServer(resolvedEnvironment.OPENCODE_SERVER_PASSWORD)),
        );
