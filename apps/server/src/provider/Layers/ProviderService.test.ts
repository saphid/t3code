...
it.effect("releases the Codex source before starting a compatible target instance", () =>
  Effect.gen(function* () {
    ...
    const registry: ProviderAdapterRegistry.ProviderAdapterRegistry["Service"] = {
      ...
      listInstances: () => Effect.succeed([sourceInstanceId, targetInstanceId]),
    ...
    yield* Effect.gen(function* () {
      ...
      const threadId = asThreadId("thread-codex-single-writer-handoff");
      const sourceSession = yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: sourceInstanceId,
        threadId,
        runtimeMode: "full-access",
        resumeCursor: { threadId: "native-thread-codex-single-writer-handoff" },
      });
      const lifecycle: Array<"source.stop" | "target.start"> = [];
      const originalSourceStop = source.stopSession.getMockImplementation();
      const originalTargetStart = target.startSession.getMockImplementation();
      assert.exists(originalSourceStop);
      assert.exists(originalTargetStart);
      source.stopSession.mockImplementation((stoppedThreadId) =>
        Effect.sync(() => lifecycle.push("source.stop")).pipe(
          Effect.andThen(originalSourceStop(stoppedThreadId)),
        ),
      );
      target.startSession.mockImplementation((input) =>
        Effect.sync(() => lifecycle.push("target.start")).pipe(
          Effect.andThen(originalTargetStart(input)),
        ),
      );
      ...
      assert.deepEqual(lifecycle, ["source.stop", "target.start"]);
      assert.equal(target.startSession.mock.calls.length, 1);
      assert.deepEqual(
        target.startSession.mock.calls[0]?.[0]?.resumeCursor,
        sourceSession.resumeCursor,
      );
      assert.equal(replacement.providerInstanceId, targetInstanceId);
