...
export const DEFAULT_DELEGATION_INSTRUCTIONS =
  ...
  "stale. When a name is repeated, pick the intended one by its listed " +
  "visible context (for example the containing thread row's title) and " +
  "occurrence, not by occurrence alone. Opening a menu or dialog first may " +
  "be required before its items are listed.\n" +
  ...
  "The run status rules below apply to continueThread as well as startThread.\n" +
  "Run status semantics for voice.startThread: the thread started only " +
  'when the returned run.status is "preparing", "queued", "starting", ' +
  '"running", or "waiting"; started means running, not finished. A ' +
  'run.status of "failed" means the worker failed: say so and speak the ' +
  'run.lastError text to the user. A run.status of "completed", ' +
  '"interrupted", "cancelled", or "rolled_back" means a worker was on the ' +
  "thread but is not running now; report that status verbatim, and announce " +
  "neither success nor failure beyond what the status states. A missing run " +
  'read-out, or a run.status of "idle", means the start could not be ' +
  "confirmed: no worker was observed running on the thread at read-back. " +
  "Say the result could not be confirmed yet, and check with " +
  "voice.observeThread before stating an outcome; do not announce success " +
  "and do not announce definite failure in any of those cases. Results that list several environments (a " +
...
const VOICE_TOOL_DESCRIPTIONS: Record<keyof typeof VoiceToolSchemas, string> = {
  ...
  listControls:
    "List the attached T3 window's activatable interface controls (buttons, links, menu items, tabs, form controls) with stable ids, including disabled and optionally hidden ones. Repeated names carry the visible container context (such as the thread row title) that tells them apart.",
  clickControl:
