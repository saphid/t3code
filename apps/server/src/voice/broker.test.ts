...
describe("voice broker routes", () => {
  ...
  it("pins the run-status semantics in the delegation instructions", () => {
    ...
    expect(DEFAULT_DELEGATION_INSTRUCTIONS).toMatch(
      /voice\.startThread: the thread started only when the returned run\.status is "preparing", "queued", "starting", "running", or "waiting"/,
    );
    expect(DEFAULT_DELEGATION_INSTRUCTIONS).toMatch(
      /run\.status of "failed" means the worker failed: say so and speak the run\.lastError text to the user/,
    );
    ...
    // Reviewer-accepted: terminal states are reported verbatim.
    expect(DEFAULT_DELEGATION_INSTRUCTIONS).toMatch(
      /"interrupted", "cancelled", or "rolled_back" means a worker was on the thread but is not running now/,
    );
