...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    copyImageAt: vi.fn(),
    getURL: vi.fn(() => "t3code-typed-swiftui-desktop-dev://app/"),
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "t3code-typed-swiftui-desktop://app/",
        navigationUrl: "t3code-typed-swiftui-desktop://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], [
          "t3code-typed-swiftui-desktop-dev://app/",
        ]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "t3code-typed-swiftui-desktop-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "t3code-typed-swiftui-desktop-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "t3code-typed-swiftui-desktop-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
