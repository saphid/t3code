...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        DesktopConfig.layerTest({ T3_TYPED_SWIFTUI_HOME: baseDir, ...env }),
      ),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { T3_TYPED_SWIFTUI_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        T3_TYPED_SWIFTUI_DESKTOP_LAN_HOST: "10.0.0.7",
        T3_TYPED_SWIFTUI_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
  ...
  it.effect("advertises loopback, LAN, and configured manual endpoints from runtime state", () =>
    withHarness(
      ...
      {
        T3_TYPED_SWIFTUI_DESKTOP_HTTPS_ENDPOINTS:
