...
describe("ElectronProtocol", () => {
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.registerDesktopProtocol({
            scheme: "t3code-typed-swiftui-desktop-dev",
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("t3code-typed-swiftui-desktop-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "t3code-typed-swiftui-desktop-dev://app",
                  referer: "t3code-typed-swiftui-desktop-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' t3code-typed-swiftui-desktop-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' t3code-typed-swiftui-desktop-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(
        ...
        ["t3code-typed-swiftui-desktop-dev"],
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["t3code-typed-swiftui-desktop-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          const protocol = yield* ElectronProtocol.ElectronProtocol;
          yield* protocol.registerDesktopProtocol({
            scheme: "t3code-typed-swiftui-desktop",
            targetOrigin: new URL("http://127.0.0.1:3773/"),
            backendOrigin: new URL("http://127.0.0.1:3773/"),
            clerkFrontendApiHostname: undefined,
          });
          return yield* Effect.promise(() =>
            handler!(new Request("t3code-typed-swiftui-desktop://other/")),
          );
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() =>
            handler!(new Request("t3code-typed-swiftui-desktop-dev://app/")),
          );
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "t3code-typed-swiftui-desktop-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(
        error.message,
        'Failed to register Electron protocol scheme "t3code-typed-swiftui-desktop-dev".',
      );
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "t3code-typed-swiftui-desktop");
        assert.strictEqual(error.cause, cause);
        assert.equal(
          error.message,
          'Failed to unregister Electron protocol scheme "t3code-typed-swiftui-desktop".',
        );
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "t3code-typed-swiftui-desktop:", "data:"]);
  });
});
