...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/T3 Typed Desktop (Dev).app",
      "T3 Typed Desktop (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "T3 Typed Desktop (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/T3 Typed Desktop (Dev).app/Contents/MacOS/T3 Typed Desktop (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/T3 Typed Desktop (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/T3 Typed Desktop (Dev).app/Contents/MacOS/Electron'",
    );
