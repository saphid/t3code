...
enum HomeEnvironmentFilter {
    enum OptionTextEmphasis: Equatable {
        case primary
        case muted
    }

    enum OptionAction: Equatable {
        case include
        case exclude
        case preserveLastIncluded
    }

    struct OptionPresentation: Equatable {
        let textEmphasis: OptionTextEmphasis
    ...
    static func projectScopeLabel(
        project: FeatureProject?,
        environmentLabels: [String: String],
        includedEnvironmentCount: Int
    ) -> String {
        guard let project else { return "All projects" }
        guard includedEnvironmentCount > 1 else { return project.name }
        guard let environmentLabel = environmentLabels[project.environmentID] else {
            return project.name
        }
        return "\(project.name) · \(environmentLabel)"
    }

    static func optionPresentation(
        isIncluded: Bool,
        includedCount: Int
    ) -> OptionPresentation {
        if !isIncluded {
            return OptionPresentation(textEmphasis: .muted, action: .include)
        }
        return OptionPresentation(
            textEmphasis: .primary,
            action: includedCount == 1 ? .preserveLastIncluded : .exclude
        )
    }

    static let addEnvironmentActionTitle = "Add Environment"
