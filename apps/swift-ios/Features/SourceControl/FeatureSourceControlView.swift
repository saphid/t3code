...
public struct FeatureSourceControlView: View {
    ...
    @State private var recovery = FeatureSourceControlRecoveryState()
    @State private var commitMessage = ""
    @State private var pendingCommitAction: FeatureSourceControlAction?
    @AccessibilityFocusState private var recoveryFocus: FeatureSourceControlRecoveryFocus?
        ...
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button { Task { await load() } } label: { Image(systemName: "arrow.clockwise") }
                    .disabled(isLoading || recovery.isBusy)
    ...
    private func failureBanner(_ failure: FeatureSourceControlFailure) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            VStack(alignment: .leading, spacing: 6) {
                Label(failure.operation.failureTitle, systemImage: "exclamationmark.triangle.fill")
            ...
            HStack(spacing: 10) {
                Button {
                    Task { await run(failure.operation) }
                } label: {
                ...
                .disabled(failure.isRetrying || recovery.isBusy)
                .accessibilityLabel(failure.operation.retryAccessibilityLabel)
    ...
    private func run(_ operation: FeatureSourceControlOperation) async {
        guard recovery.begin(operation) else { return }
        if operation.isLoad {
            isLoading = true
        }
        defer {
        ...
        do {
            ...
            case .load:
                ...
                recovery.finishSuccess(operation)
            case let .action(action, message):
                ...
                do {
                    status = try await client.sourceControlStatus(threadID: threadID)
                    recovery.finishSuccess(operation, alsoSatisfying: .load)
                } catch {
                    recovery.finishFollowUpFailure(
                        after: operation,
                        retrying: .load,
                        error: error
                    )
            ...
            recovery.finishFailure(operation, error: error)
        }
    }
}
