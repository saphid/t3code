...
struct HomeEnvironmentFilterTests {
    ...
    @Test
    func projectScopeUsesEnvironmentOnlyWhenMultipleAreIncluded() {
        let selectedProject = project(id: "shared", environmentID: "studio")

        #expect(
            HomeEnvironmentFilter.projectScopeLabel(
                project: selectedProject,
                environmentLabels: ["studio": "Studio Mac"],
                includedEnvironmentCount: 1
            ) == "shared"
        )
        #expect(
            HomeEnvironmentFilter.projectScopeLabel(
    ...
    @Test
    func optionPresentationKeepsSelectedTextPrimaryWithoutDisablingTheLastChoice() {
        #expect(
            HomeEnvironmentFilter.optionPresentation(isIncluded: true, includedCount: 2)
                == .init(textEmphasis: .primary, action: .exclude)
        )
        #expect(
            HomeEnvironmentFilter.optionPresentation(isIncluded: true, includedCount: 1)
                == .init(textEmphasis: .primary, action: .preserveLastIncluded)
        )
        #expect(
            HomeEnvironmentFilter.optionPresentation(isIncluded: false, includedCount: 1)
                == .init(textEmphasis: .muted, action: .include)
        )
        #expect(HomeEnvironmentFilter.addEnvironmentActionTitle == "Add Environment")
    }
