...
struct NativePassiveLiveShellTests {
    ...
    func unknownEventRepairsWithoutPollingDelay() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: fixture.directory) }
        let initialHTTP = PassiveRequestGate()
        let held = PassiveRequestGate()
        await fixture.transport.holdNextShell(host: "two.example", gate: initialHTTP)
        do {
            _ = try await fixture.client.initialSnapshot()
            let threadID = FeatureScopedID.thread(environmentID: "two", wireID: "thread-two")
            await server.waitForSubscriptions(host: "two.example", count: 1)
            try await initialHTTP.waitUntilEnteredCancellable()
            // Establish this socket epoch before draining the held startup read.
            // A mere httpFinished receipt may otherwise describe a rejected read
            // whose replacement races with the unknown-event repair.
            ...
            try await receipts.waitForShells("two", count: 1)
            await initialHTTP.release()
            try await receipts.waitForHTTP("two", count: 1)
            #expect(await fixture.transport.shellReadCount(host: "two.example") == 1)
