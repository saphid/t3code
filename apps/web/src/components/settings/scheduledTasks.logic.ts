...
export function buildScheduledTaskUpdateInput(
  ...
  if (!sameWorkspaceStrategy(workspaceStrategy, workspaceStrategyFromDraft(baseline))) {
    // The dialog cannot express every strategy field (`branch`), so a dirty
    // workspace control patches the fields it owns while fields it cannot
    // reach are carried over from the live task.
    patch.workspaceStrategy =
      task.workspaceStrategy.type === workspaceStrategy.type
        ? { ...task.workspaceStrategy, ...workspaceStrategy }
        : workspaceStrategy;
  }
