...
describe("buildScheduledTaskUpdateInput", () => {
  ...
  it("preserves concurrent writes to workspace fields the edit left untouched", () => {
    // The baseline snapshot goes stale the moment the dialog opens: another
    // client retargeted the branch and flipped the origin flag. The update
    // replaces the whole strategy object, so the patch is built on the live
    // strategy — only the user's base-ref edit is overlaid.
    const opened: ScheduledTask = {
      ...task,
      workspaceStrategy: {
        type: "worktree",
        baseRef: "main",
    ...
    const baseline = taskToDraft(opened);
    const draft = { ...baseline, baseRef: "develop" };
    expect(
      buildScheduledTaskUpdateInput(
        draft,
...
describe("moveDetachesThreadBinding", () => {
  ...
  it("detaches explicitly on a project move even when the draft was never bound", () => {
    const opened = taskToDraft(task);
    const moved = { ...opened, projectId: "project:two" };
    // No detach hint: the draft's binding never changed. But the patch still
    // sends threadId: null — a binding another client added after the dialog
    // opened belongs to the old project and must not be carried across.
    expect(moveDetachesThreadBinding(moved, opened)).toBe(false);
    const patch = buildPatch(moved, task, opened);
    expect(patch).toMatchObject({ threadId: null });
  });
});
