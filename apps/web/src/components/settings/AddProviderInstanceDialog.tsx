...
export function AddProviderInstanceDialog({
  ...
  useEffect(() => {
    ...
    setSaveError(null);
    return () => {
      // The RPC may finish after this form closes or changes environments.
      pendingSave.current = null;
    };
  }, [open, environmentId]);

  const changeOpen = (nextOpen: boolean) => {
    if (!nextOpen) pendingSave.current = null;
  ...
  const handleSave = async () => {
    if (!open || pendingSave.current !== null) return;
    setHasAttemptedSubmit(true);
    ...
    try {
      ...
      // Failure includes interruption: only an explicit success confirms the save.
          ...
          <fieldset disabled={isSaving} className="contents">
            <div className={cn("grid gap-2", wizardStep !== 0 && "hidden")}>
              ...
              <RadioGroup
                value={driver}
                onValueChange={(value) => {
                  if (pendingSave.current === null) setDriver(ProviderDriverKind.make(value));
                }}
            ...
            <label className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
              ...
              <Input
                ...
                value={label}
                onChange={(event) => {
                  if (pendingSave.current === null) setLabel(event.target.value);
                }}
              />
            ...
            <label className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
              ...
              <Input
                ...
                value={instanceId}
                onChange={(event) => {
                  if (pendingSave.current === null) setInstanceIdOverride(event.target.value);
                }}
            ...
            <div className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
              ...
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                <ProviderAccentColorPicker
                  ...
                  onCommit={(value) => {
                    if (pendingSave.current === null) setAccentColor(value);
                  }}
                  layout="inline"
                />
