...
const finishedObservation = (threadId: ThreadId): VoiceObserveThreadOutput => ({
  ...
  threadId,
  sequence: 50,
  run: { status: "completed", lastError: null },
  runId: run1,
  running: false,
});
  ...
  const noListRefsHost = () => {
    const access = {
      ...
      launchThread: vi.fn(async () => {
        throw new Error("must not launch: the missing listRefs boundary refuses first");
      }),
    };
    return {
      catalogEnvironments: () => [
...
describe("voice module wiring", () => {
  ...
  it("defers delivery (steer returns false) and recovers it after the next session goes live", async () => {
    ...
    expect(stored[0]!.deliveredRunIds).toHaveLength(0);
    ...
    expect(delivered[0]!.deliveredRunIds).toHaveLength(1);
...
describe("voice module disposal", () => {
  it("dispose leaves a pending delivery watching (at-least-once preserved), and reconnect after the disposal cycle still recovers delivery once", async () => {
    ...
    expect(storedAfterDispose[0]!.deliveredRunIds).toHaveLength(0);
    ...
    expect(storedAfterRecovery[0]!.deliveredRunIds).toHaveLength(1);
  });
});
