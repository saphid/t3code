  ...
  it("records the resolved stale-session response shape with dispatch identity and session state", async () => {
    ...
    await recorder.recordToolExecution(
      ...
      async () => {
        ...
        return {
          ...
          messageId: "msg_1",
          dispatchSequence: 42,
          run: { status: "failed", lastError: "provider session stale; start a new thread" },
    ...
    expect(entry).toEqual({
      ...
      result: {
        ...
        runStatus: "failed",
        runLastError: "provider session stale; start a new thread",
      },
      startedAt: 1000,
      endedAt: 1400,
    });
    ...
    const exported = JSON.parse(exportVoiceHistory(reloaded, () => 9000)) as {
      sessions: Array<{ entries: Array<{ result?: { runLastError?: string } }> }>;
    };
    expect(exported.sessions[0]!.entries[0]!.result!.runLastError).toBe(
