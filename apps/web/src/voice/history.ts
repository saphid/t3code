 ...
 * sequence, run status and lastError, acknowledged and destination,
...
const decodeToolResult = (value: unknown): VoiceHistoryToolResult | undefined => {
  ...
  const runStatus = string("runStatus");
  const runLastError = string("runLastError");
  ...
  const result: VoiceHistoryToolResult = {
    ...
    ...(runLastError !== undefined ? { runLastError } : {}),
...
export function sanitizeToolResult(output: unknown): VoiceHistoryToolResult | undefined {
  ...
  const runStatus = run === undefined ? undefined : asString(run.status);
  const rawRunLastError = run === undefined ? undefined : (run.lastError as unknown);
  const runLastError = typeof rawRunLastError === "string" ? capText(rawRunLastError) : undefined;
  ...
  const result: VoiceHistoryToolResult = {
    ...
    ...(messageId !== undefined ? { messageId } : {}),
    ...(dispatchSequence !== undefined ? { dispatchSequence } : {}),
    ...(runStatus !== undefined ? { runStatus: capText(runStatus) } : {}),
    ...(runLastError !== undefined ? { runLastError } : {}),
    ...(controls !== undefined ? { controls } : {}),
  };
  return Object.keys(result).length > 0 ? result : undefined;
