...
const startReceipt = (overrides?: {
  ...
  run: { status: "running", lastError: null },
});
...
describe("delegate", () => {
  it("resolves the thread/task receipt immediately without blocking on the worker", async () => {
    ...
    expect(receipt.run).toEqual({ status: "running", lastError: null });
...
// ---------------------------------------------------------------------------
// Observation, exactly-once delivery, (threadId, runId) keying
// ---------------------------------------------------------------------------

describe("observation and delivery (fallback channel)", () => {
  it("delivers the result once, keyed by (threadId, runId), with afterSequence resume", async () => {
    const harness = makeBridge({
      observeOutputs: [runningObservation(43), finishedObservation(44, run1)],
    });
    await harness.bridge.delegate({
      requestId,
      environmentId: envA,
    ...
    expect(record?.deliveredRunIds).toEqual([`${threadA}:${run1}`]);
  ...
  it("never delivers twice for duplicate terminal events on the same turn", async () => {
    const harness = makeBridge({
      observeOutputs: [
        finishedObservation(43, run1),
        finishedObservation(44, run1),
        finishedObservation(45, run1),
      ],
    });
    await harness.bridge.delegate({
      requestId,
  ...
  it("delivers a second turn on the same thread separately under its own key", async () => {
    ...
    const [record] = await savedRecords(harness.store);
    expect(record?.deliveredRunIds).toEqual([`${threadA}:${run1}`, `${threadA}:${run2}`]);
    harness.bridge.dispose();
  });
  ...
  it("defers delivery when the voice session cannot steer and delivers exactly once after reconnect", async () => {
    const harness = makeBridge({
      steerResult: false,
      observeOutputs: [finishedObservation(43, run1)],
    });
    await harness.bridge.delegate({
      requestId,
      environmentId: envA,
    ...
    harness.executor.observeThread.mockImplementation(async () => finishedObservation(43, run1));
  ...
  it("delivers once after a full client restart via the persisted pending mapping", async () => {
    ...
    // Already delivered under the persisted (threadId, runId) key.
