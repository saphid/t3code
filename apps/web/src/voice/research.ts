 ...
 * (threadId, runId) — the frozen delivery key, never repeated session
...
/** A persisted pending-delivery mapping entry, keyed by the VoiceRequestId.
    ...
    request id), while server-owned facts (runId, event sequence) are
...
export interface VoiceDeliveryRecord {
  ...
  /** Delivered (threadId, runId) keys — the exactly-once ledger. */
  readonly deliveredRunIds: string[];
  lastError?: string;
}
...
function cloneRecord(record: VoiceDeliveryRecord): VoiceDeliveryRecord {
  return { ...record, deliveredRunIds: [...record.deliveredRunIds] };
}

const deliveryKey = (threadId: ThreadId, runId: RunId): string => `${threadId}:${runId}`;
...
export function createResearchBridge(
  ...
  /** Reads the delivered thread identity's final result and steers it into
      ...
      content and the (threadId, runId) key is recorded; "deferred" means
  ...
  const deliverTurnResult = async (
    ...
    const key = deliveryKey(record.threadId, runId);
    if (record.deliveredRunIds.includes(key) || delivering.has(key)) {
      return "delivered";
    }
    delivering.add(key);
    try {
      ...
      const turn =
        [...turns].filter((candidate) => candidate.runId === runId).at(-1) ??
      ...
      if (resultText === null) {
        ...
        content =
          ...
          `run ${runId}). Result: ${resultText} If you need more detail, read that ` +
      ...
      record.deliveredRunIds.push(key);
  ...
  const runObservationStep = async (record: VoiceDeliveryRecord): Promise<boolean> => {
    ...
    if (observation.run.status === "failed") {
      ...
      const sent =
        steer === undefined ? false : steer(workerErrorNotice(observation.run.lastError));
      reportWorkerError(record, observation.run.lastError, observation.runId, sent);
      return false;
    }

    if (!observation.running && observation.runId !== undefined) {
      const outcome = await deliverTurnResult(record, observation.runId);
