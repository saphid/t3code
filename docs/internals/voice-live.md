...
Delivery is keyed by thread ID and run ID, never by repeated run status, and reconnect
...
`continueThread` dispatches a V2 `message.dispatch` to the supplied existing
thread ID, with no bootstrap or title seed. It reads the thread before
dispatch and keeps its model, runtime and interaction mode; delivery resolves
through the server's serialized run state where the server resolves command
context, and client-side otherwise. Missing targets fail without creation.
