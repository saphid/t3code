...
| 6 | `shared-electron-vscode-themes` | One fixture journey searches Open VSX, installs a compatible theme, selects both modes, checks exact native and terminal color-role values, relaunches, verifies persistence, and captures light, dark, and terminal proof checkpoints with tap events | The installed Test build still needs clean and annotated media across the real theme service and representative native and terminal screens |
...
| 8 | `safe-tool-content-recovery` | One fixture journey forces a Source Control refresh failure, keeps the last branch and file visible, exposes an identified Retry control, checks single-row rendering, retries, clears the notice, and sets accessibility focus back to the branch | VoiceOver focus and clean and annotated recovery media still need verification on the installed Test build |
| 9 | `development-build-source-thread` | One fixture journey proves exact revision, summary, and commit attribution, then records the source-thread tap and checks that the existing expected conversation opens once | The installed Test build still needs proof with a real embedded changelog and its source thread |
