...
// ---------------------------------------------------------------------------
// Shared run-state vocabulary
// ---------------------------------------------------------------------------

/** The V2 run state a voice readout reports, with `idle` for a thread that
    has no runs (or no live run to report). Voice has no long-lived session on
    Orchestrator V2: the latest run's status is the thread's worker state. */
export const VoiceThreadRunStatus = Schema.Union([
  Schema.Literal("idle"),
  OrchestrationV2RunStatus,
]);
export type VoiceThreadRunStatus = typeof VoiceThreadRunStatus.Type;
...
export const VoiceThreadTurnExcerpt = Schema.Struct({
  /** The V2 run this turn belongs to, when the read could attribute it. */
...
export const VoiceThreadExcerpt = Schema.Struct({
  ...
  createdAt: Schema.optional(Schema.String),
  updatedAt: Schema.optional(Schema.String),
  /** Latest run state, for "is this thread still running" answers. */
  runStatus: Schema.optional(VoiceThreadRunStatus),
  runLastError: Schema.optional(Schema.String),
...
/** Read-back of the target thread's latest run after a start or continue
    dispatch. Present once the read-back completed. Started means a run was
    observed active (`preparing`/`queued`/`starting`/`running`/`waiting`);
    `failed` means the worker failed (see `lastError`); other terminal
    statuses mean a worker ran on the thread but is not running now; `idle`
    means the start could not be confirmed at read-back. */
export const VoiceThreadRunReadout = Schema.Struct({
  status: VoiceThreadRunStatus,
  lastError: Schema.NullOr(TrimmedString),
});
export type VoiceThreadRunReadout = typeof VoiceThreadRunReadout.Type;
...
export const VoiceStartThreadOutput = Schema.Struct({
  ...
  /** Read-back of the thread's latest run after dispatch. Thread existence
      alone is never reported as successful execution. A read-back that still
      shows the pre-dispatch state describes the previous turn (the new run is
      only projected once the server records it); continueThread reports such
      a follow-up as `starting` instead of echoing a stale terminal status or
      error. */
  run: Schema.optional(VoiceThreadRunReadout),
});
...
export const VoiceObserveThreadOutput = Schema.Struct({
  ...
  /** Identity of the in-flight or latest completed run, when known. Research
      delivery is keyed by (threadId, runId), never by status repetition
      alone. */
  runId: Schema.optional(RunId),
