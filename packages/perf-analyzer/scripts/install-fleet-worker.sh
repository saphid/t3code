...
os="$(uname -s)"
worker_user="${T3_PERF_WORKER_USER:-${SUDO_USER:-$(id -un)}}"
...
if [ "$os" = Linux ]; then
  ...
  sudo install -d -m 0750 -o "$worker_user" -g docker "$data_dir"
  sudo install -d -m 0750 -o "$worker_user" -g docker "$data_dir/docker-config"
