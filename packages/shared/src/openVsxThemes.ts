...
export async function getOpenVsxThemeExtension(
  extensionId: string,
  signal?: AbortSignal,
): Promise<OpenVsxThemeExtension> {
  const separator = extensionId.indexOf(".");
  const namespace = extensionId.slice(0, separator).trim();
  const name = extensionId.slice(separator + 1).trim();
  if (separator <= 0 || !namespace || !name || !/^[a-z0-9._-]+$/i.test(namespace + name)) {
    throw new Error("That Open VSX extension identifier is invalid.");
  }
  return withSearchTimeout(async (requestSignal) => {
    ...
    const response = await fetch(url, { signal: requestSignal });
    if (!response.ok) throw new Error("That Open VSX theme is unavailable.");
    ...
    if (!extension || extension.id.toLowerCase() !== extensionId.toLowerCase()) {
      throw new Error("That Open VSX extension is not a supported color theme.");
    }
    return extension;
  }, signal);
}
