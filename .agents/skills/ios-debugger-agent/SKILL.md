...
Do not assume an installed React Native build verifies SwiftUI behavior, or vice versa. Use [`test-t3-mobile`](../test-t3-mobile/SKILL.md) for the full isolated-backend and pairing workflow. The SwiftUI Debug bundle is `com.alxs.t3code.typed-swiftui.dev`; never substitute the existing `com.t3tools.t3code.swiftui.dev` app.
